...
#var debug_mode: bool = Globals.debug_mode
var game_paused: bool = false
#var game_paused: bool = Globals.game_paused
var time_stamp = null
#var time_stamp: bool = Globals.time_stamp
...
func _exit_tree() -> void:
	...
	# [DEBUG] Message
	if debug_mode: print(time_stamp, " [DEBUG] '", get_script().resource_path.get_file().get_basename(), "' scene unloaded.")
...
func _ready() -> void:
	...
	# [DEBUG] Message
	if debug_mode: print(time_stamp, " [DEBUG] '", get_script().resource_path.get_file().get_basename(), "' scene loaded.")
...
func update_velocity(delta: float) -> void:
	...
	# Get the input direction and handle the movement/deceleration. # -1 left, 0 middle , 1 right.
	var direction = Input.get_axis("move_left", "move_right") # Joystick
	if direction == 0.0:
		direction = Input.get_axis("dpad_left", "dpad_right") # D-pad
